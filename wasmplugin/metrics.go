...
func (m *wafMetrics) CountTXInterruption(phase string, ruleID int, metricsLabels []map[string]string) {
	...
	for _, metricsLabel := range metricsLabels {
		sb.WriteString(fmt.Sprintf("_%s=%s", metricsLabel["key"], metricsLabel["value"]))
	}
